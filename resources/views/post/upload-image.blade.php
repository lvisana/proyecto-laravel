        ...
        <section class="py-12 container text-light">
            <div class="row max-w-7xl justify-content-center p-4 sm:p-8 dark:bg-gray-800 shadow sm:rounded-lg mx-auto sm:px-6 lg:px-8 space-y-6">
                <div class="card-body">
                    ...
                    <form method="POST" action="@if (isset($image)) {{route('image.update', ['id' => $image->id])}} @else {{route('image.save')}} @endif" class="d-flex flex-column gap-3" enctype="multipart/form-data">
                        @csrf
                        ...
                        @if (isset($image))
                            <div class="row justify-content-center">
                                ...
                                <div class="col-md-7">
                                    <img src="{{route('image.file', ['filename' => $image->image_path])}}" style="height: 300px;" alt="">
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="form-group row align-items-center justify-content-center">
                            ...
                            <div class="col-md-7">
                                <textarea type="file" name="description" id="description" class="rounded border border-1 border-light w-100 p-4 text-light" required>@if (isset($image)) {{$image->description}} @endif</textarea>
                        ...
                        <div class="col-md-12 text-center">
                            <x-primary-button>@if (isset($image)) {{ __('Edit') }} @else {{ __('Upload') }} @endif</x-primary-button>
                        </div>
